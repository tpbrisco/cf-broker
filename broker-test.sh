...
INSTANCE_ID=$(uuidgen)
curl ${CURL_FLAGS} -sSf -u user:pass -X PUT \
...
echo "Checking service instance $SVC_ID for plan $PLAN_ID"
FETCH_ID=$(curl ${CURL_FLAGS} -sSf -u user:pass -X GET \
		-H 'X-Broker-API-Version: 2.10' \
		${URL}/v2/console | \
...
echo Deleting service instance $INSTANCE_ID binding $BIND_ID
curl ${CURL_FLAGS} -sSf -u user:pass -X DELETE \
     -H 'X-Broker-API-Version: 2.10' \
     -o /dev/null \
     ${URL}/v2/service_instances/${INSTANCE_ID}/service_bindings/${BIND_ID}
...
for guid in $(curl ${CURL_FLAGS} -sSf -X GET ${URL}/v2/console | jq '.instances[].id');
do
    echo "  $guid"
